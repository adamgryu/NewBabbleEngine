...
namespace BabbleEngine
{
    ...
    public class RoomEditor : Room
    {
        ...
        {
            if (Input.KeyboardTapped(Keys.Up) && blockSize.Y > 16)
                blockSize.Y -= 16;
            if (Input.KeyboardTapped(Keys.Down))
                blockSize.Y += 16;
            if (Input.KeyboardTapped(Keys.Left) && blockSize.X > 16)
                blockSize.X -= 16;
            if (Input.KeyboardTapped(Keys.Right))
                blockSize.X += 16;

            if (((int)blockSize.X) == 32 && ((int)blockSize.Y) == 32)
                snapSize = 32;
            else
                snapSize = 16;
            ...
            if (Input.MouseLeftButtonDown)
            {
                if (!Input.IsKeyDown(Keys.LeftControl))
                {
                    ...
                    if (b == null)
                    {
                        blocks.Add(new Block(snapMouse, blockSize, Input.IsKeyDown(Keys.A)));
                    }
                }
                else
                {
